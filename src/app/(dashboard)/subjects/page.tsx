...
export default async function SubjectsPage() {
  ...
  if (deptIds.length === 0) return <div><h2 className="text-2xl font-bold mb-4">Subjects</h2><p>Please create a Department first.</p></div>;

  const { data: courses } = await supabase.from("courses").select("id").in("department_id", deptIds);
  const courseIds = courses?.map(c => c.id) || [];
  if (courseIds.length === 0) return <div><h2 className="text-2xl font-bold mb-4">Subjects</h2><p>Please create a Program first.</p></div>;
  ...
  if (semIds.length === 0) return <div><h2 className="text-2xl font-bold mb-4">Subjects</h2><p>Please create a Semester first.</p></div>;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage subjects for semesters.</p>
        </div>
      </div>
      <SubjectsClient 
        data={subjects || []} 
